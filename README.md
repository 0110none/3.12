...
本项目已提供 **PyInstaller 正式构建脚本**，用于生成发布目录（适合交付到值班室工位）。

```bash
./scripts/build_release.sh
```

构建完成后输出目录：`dist/安防监控告警系统/`。

发布建议：
- 将 `dist/安防监控告警系统` 整体打包为 zip 后下发；
- 首次部署时保留 `config/` 目录，便于现场调整摄像头；
- 生产环境建议固定分辨率 1920x1080 或 2560x1440，默认全屏启动。
